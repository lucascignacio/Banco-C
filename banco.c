...
void criarConta(){
	...
	//Data do cadastro
	char dia[3]; //06\0 por ser uma string.
	char mes[3];
	char ano[5];
	...
	strcat(data_cadastro, dia); // D
	strcat(data_cadastro, "/"); // D/
	strcat(data_cadastro, mes); // D/M
	strcat(data_cadastro, "/"); // D/M/
	strcat(data_cadastro, ano); // D/M/A
	strcat(data_cadastro, "\0"); //Finalizando a String.
		...
		for(int i = 0; i < contador_contas; i++){
			if(contas[i].numero == conta.numero){
				if(contas[i].saldo >= valor){
					...
					printf("Saque efetuada com sucesso\n");
				} else {
				...
				for(int cd = 0; cd < contador_contas; cd++){
					if(contas[cd].numero == conta_destino.numero){
						if(contas[co].saldo >= valor){
							...
							contas[cd].saldoTotal = atualizaSaldoTotal(contas[co]);
							contas[cd].saldoTotal = atualizaSaldoTotal(contas[cd]);
							printf("Transferência realizada com sucesso!\n");
						}
					}
...
void efetuarSaque(){
	if(contador_contas > 0){
		...
		printf("Ainda não existem contas pra saque.\n");
	}
	sleep(2);
	menu();
}
...
void efetuarDeposito(){
	if(contador_contas > 0){
			...
			printf("Ainda não existem contas pra depósito.\n");
		}
		sleep(2);
		menu();
}
...
void efetuarTransferencia(){
	if(contador_contas > 0){
		int numero_o, numero_d; // Origem e Destino.
		...
		printf("Ainda não existe contas para transferências.\n");
	}
	sleep(2);
	menu();
}
